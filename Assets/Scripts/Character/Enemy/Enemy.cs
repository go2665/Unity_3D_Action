...
public class Enemy : RecycleObject, IBattler, IHealth
{
    ...
    protected override void OnEnable()
    {
        ...
        rigid.linearDamping = Mathf.Infinity;        // 무한대로 되어 있던 마찰력을 낮춰서 떨어질 수 있게 하기
        HP = maxHP;                         // HP 최대로
    ...
    IEnumerator DeadSquence()
    {
        ...
        rigid.linearDamping = 10.0f;                     // 무한대로 되어 있던 마찰력을 낮춰서 떨어질 수 있게 하기
    ...
    void MakeDropItems()
    {
        ...
        foreach(var item in dropItems)
        {
            if( item.dropRatio > UnityEngine.Random.value ) // 확률 체크하고
            {
                ...
                GameObject[] items = Factory.Instance.MakeItems(item.code, count, transform.position, true); // 실제 생성
                //foreach (var obj in items)
                //{
                //    Debug.Log($"드랍 아이템 : {obj.name}, 위치 : {obj.transform.position}");
                //}
            }
        }
    }
